...
- A PostgreSQL migration-owner connection and permission to create two
  least-privilege login roles
...
As the migration owner, create separate operator and application logins. Set
real passwords through your database provider or secret tooling rather than
...
Give each login only the role shown above, then supply its connection through
your normal secret manager. These illustrative local URLs keep the two trust
boundaries explicit:
...
Go applications install the versioned module. The Python package still owns
the migration CLI, so every SDK uses the same ordered SQL baseline:
...
- Read [Configuration and catalog revisions](./concepts/configuration.mdx) before adding plans, quotas, entitlements, or payment offers
