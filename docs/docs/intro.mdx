...
```text
Bursar
├── credits   -> credit lifecycle: balances, ledger, leases
├── catalog   -> active versioned config
├── accounts  -> signup: default plan + grant programs
├── billing   -> provider events, subscriptions, invoices (optional)
└── commerce  -> checkout, offers, auto-recharge (optional)
        │
        ▼
PostgreSQL: one schema, per-tenant RLS, append-only ledger
```
