...
| Pricing | `get_active_pricing()`, `set_active_pricing(config, label?)`, `get_pricing_history()`, `get_bursar_config(version)`, `activate_pricing(version)` |
...
Run all 14 grouped bundled SQL migrations (idempotent). Pass the connection string from the environment rather than hard-coding it.
