...
This page is a decision guide for wiring ducto to a payment provider's webhooks (Stripe, Paddle, Chargebee, or anything else that fires an event on signup and on subscription renewal). It does not cover leases or admission control — see [Financial Safety](/docs/financial-safety) for that — it is specifically about **when to grant credits, when to configure an allowance instead, and how to make both safe under webhook redelivery**.

Read this alongside [Architecture: Scope & boundaries](/docs/architecture): **ducto has no payment-provider integration and never will.** It does not talk to Stripe, does not parse webhook signatures, and does not convert currency to credits. Every example below assumes you have already verified the webhook and decided what it means — ducto's job starts at "grant this many credits" or "assign this plan," not before.
...
```python
from ducto import FeatureLimitReachedError
