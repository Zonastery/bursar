...
The bundled SQL files create the required schema. Selected highlights (not the full list — see the `sql/` directory for every numbered migration):
...
| `023_credit_tiers.sql` | `credit_tiers` config table, `user_credit_tiers` per-user-per-tier balances, tier-aware rewrites of `credits_add`/`deduct_with_allowance`/`settle_lease`/`refund_credits`/`expire_credits`, new `get_user_credit_tiers` RPC |
