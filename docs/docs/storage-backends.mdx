...
| File | Creates |
|------|---------|
| `001_schema.sql` | Bursar schema and shared enum types |
| `010_tables_and_relations.sql` | Tables, constraints, indexes, and foreign keys |
| `020_security.sql` | Policies and row-level security |
| `030_credit_functions.sql` | Core credit, team, allowance, cap, and expiry functions |
| `040_billing_and_lease_functions.sql` | Billing, lease, catalog, and plan lifecycle functions |
| `050_triggers_and_lifecycle.sql` | Triggers, auto-recharge, and subscription lifecycle |
| `060_privileges.sql` | Function and table execution ACLs |
| `070_hardening_and_catalog.sql` | Compatibility hardening and catalog validation |

The migration ledger makes setup idempotent: each bundled file is applied once and recorded. Migrations return a `SetupResult` with `tables_created` (list of applied files), `rpcs_created`, and `errors`. Check `result.success` (true when `errors` is empty) before going further.
