...
`create_checkout` resolves the offer, enforces quantity bounds and existing
subscriptions, records a checkout intent, and asks the provider for a session
URL. The user pays on the provider's site; the provider webhook arrives as a
`checkout.completed` event and completes the intent.

```python
from bursar import Bursar, CommerceOptions, CreateCheckoutInput, PostgresBillingStore, PostgresStore
...
bursar = Bursar.create(
    ...
    commerce_options=CommerceOptions(
        ...
        providers={
            "stripe": lambda ctx: StripeProvider(
                sink=ctx.event_sink,
...
result = await bursar.commerce.create_checkout(
    CreateCheckoutInput(
        subject_id=user_id,
...
const bursar = new Bursar({
  ...
  commerceOptions: {
    ...
    providers: {
      stripe: (ctx) => new StripeProvider(() => stripe, ctx.eventSink, webhookSecret),
    },
  },
});

const result = await bursar.commerce.createCheckout({
  subjectId: userId,
  offerKey: "credits_10k",
  quantity: 2,
  returnUrl,
  cancelUrl,
...
Webhook events also flow through `ingest_billing_event`, so a payment that
never touches your checkout endpoint still settles credits. On
`payment.succeeded` for a topup, the offer's credits are granted to the
configured bucket; on `checkout.completed` for a subscription, the cycle grant
posts and the plan is assigned.
...
Billing state persists in a `PostgresBillingStore` that must be constructed
**alongside** the credit store — the two stores share the same database,
tenant, and migrations, and billing only exists when both are passed to
`Bursar.create(credit_store=..., billing_store=..., commerce_options=...)`.
Without `billing_store`, `billing` and `commerce` are `None` on the facade and
`ingest_billing_event` raises.

## Related

- [Canonical configuration](./configuration.mdx) — the `commerce` section in the full document
- [Billing and subscriptions](../guides/subscription-integration.mdx) — wiring the stores
- [Plans, entitlements, quotas, and admission](./plans.mdx) — what a subscription entitles
