...
canonical config and are enforced by the database — policy is never just
application code.
...
| `credit_allowance` | Free credits per window, consumed before the balance |
...
Allowance credits are consumed **before** the balance is debited: a deduction
first draws down `allowance_remaining`, and `DeductionResult.allowance_consumed`
reports how much of the charge the allowance covered. `check_allowance(user_id)`
returns the current window (`plan_id`, `allowance_remaining`, `period_start`,
`period_end`) for display and gating. `unset_user_plan` pauses the allowance
window; reassigning re-anchors it.
...
await bursar.accounts.onAccountCreated({ accountId: userId, eventKey: "signup:1" });

bursar.credits.setUserPlan(userId, "pro");
const allowance = bursar.credits.checkAllowance(userId);
console.log(allowance.allowanceRemaining);

const feature = bursar.credits.checkFeature(userId, "voice_mode");
...
- [Credit lifecycle](../notebooks/04_credit_lifecycle.mdx) — end-to-end walkthrough
- API reference: [Credits service (Python)](../python-api/credit-manager.mdx) and [Credits service (JavaScript)](../javascript-api/credit-manager.mdx)
