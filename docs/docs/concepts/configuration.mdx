...
Plans define when an activated revision reaches assigned accounts. `immediate` applies the new revision at activation, `next_renewal` waits for the next subscription renewal, and `pinned` keeps the assigned revision until an explicit migration.
