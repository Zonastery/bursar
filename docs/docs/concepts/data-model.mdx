...
`requested`, `charged`, `allowance_requested`, `allowance_covered`, and the
catalog revision, plan, and rate card in effect. The measures, model,
dimensions, and the full pricing snapshot live in the joined
`usage_charge_payloads` row — payload storage is separate so the charged
amount never bloats with reconciliation receipts. The `charged + allowance_covered
...
| `credit_usage_charges`     | Metered charges: operation, amounts, allowance split, rate card      |
| `usage_charge_payloads`    | Measures, model, dimensions, and pricing snapshot per charge         |
