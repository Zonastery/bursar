...
description: Accounts, the append-only ledger, lots, leases, and allowances — every concept mapped to tables on one PostgreSQL schema.
---
...
- [Canonical configuration](./configuration.mdx) — the document that defines buckets, lots, and plans
- [Financial safety](../guides/financial-safety.mdx) — holds, leases, and the money invariants in action
- [Manage the credit lifecycle](../notebooks/credits-and-controls/04_credit_lifecycle.mdx) — every entry kind on one ledger
