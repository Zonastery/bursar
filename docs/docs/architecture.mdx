...
├── config.py            # Pydantic model + dict loading for BursarConfig
...
- **Core (required):** credit balance / add / deduct (`get_balance`, `add_credits`, `deduct_with_allowance`), the full lease lifecycle (`create_lease` / `settle_lease` / `release_lease` / `renew_lease` / `get_available`), pricing config versioning (`get_active_pricing`, `set_active_pricing`, `get_pricing_history`, `get_bursar_config`, `activate_pricing`), plan management (`set_user_plan`, `get_user_plan`, `check_feature`, `check_allowance`), spend cap checks (`check_spend_cap`), refunds (`refund_credits`), and the credit expiry sweep (`sweep_expired_credits`).
