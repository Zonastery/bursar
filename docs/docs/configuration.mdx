...
| `feature_limits` | object | no | Per-feature invocation-count limits, keyed by feature name. Independent of `features` — a feature can be entitled *and* rate-limited at the same time. See [`feature_limits`](#feature_limits) below. |
...
| `action` | `"deny"` \| `"warn"` \| `"notify"` | no | What happens once `max_calls` is reached within the window. Default: `"deny"`. Mirrors `SpendCap.action` both in name and in semantics — see [Architecture: Credit Lifecycle](/docs/architecture#credit-lifecycle) for how `deny`/`warn`/`notify` work for spend caps. |
...
4. Plan names are unique (and every plan has a `name`)
5. Plan `rate_overrides` expressions are valid
6. No unknown keys, at any level (top-level, `plans.*`, `tiers.*`, `plans.*.feature_limits.*`, `plans.*.per_operation.*`) — a typo like `min_balnce` is rejected rather than silently falling back to a default
...
9. `min_balance`, `signup_bonus`, `fixed.*`, and `plans.*.free_allowance` must be `>= 0`
