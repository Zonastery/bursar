...
A tier named `"allowance"` is unrelated to `PlanDefinition.free_allowance` (the per-plan monthly subscription quota consumed via `credit_usage_window`, before `net` is computed — see [Plan fields](#plan-fields)). They're independent systems; ducto doesn't reserve the name, but a tier called `"allowance"` is a documentation footgun, not a technical conflict.
:::

### No-tiers fallback

Omitting `tiers` entirely preserves 100% of today's single-balance behavior: internally, ducto routes everything through a synthetic `"default"` tier, so every existing config keeps working with zero changes. An explicit empty `tiers: {}` is a config validation error — it's ambiguous whether you meant "no tiers configured" or "tiers not set up yet" — omit the key instead.
