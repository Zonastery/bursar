...
const refund = await bursar.credits.refundCredits(charge.entryId, undefined, "clawback");
```
...
`addCredits` and `refundCredits` accept an `idempotencyKey` for replay-safe
retries: retrying with the same key replays the original result instead of
double-applying.

**`deductCredits` does not accept an `idempotencyKey`** — its options are
`entryType`, `bucket`, and `metadata` only. It is an administrative adjustment
(a refund clawback), not the metered path; do not build retries on it. For
replay-safe charging, use `deduct(userId, metrics, idempotencyKey)` — the
high-volume, idempotency-keyed path (see below).
...
| `deductCredits` | `userId: string`, `amount: Decimal \| number`, `options?: { entryType?, bucket?, metadata? }` | `Promise<AddCreditsResult>` |
| `refundCredits` | `entryId: string`, `amount?`, `reason?`, `metadata?`, `idempotencyKey?` | `Promise<RefundResult>` |
...
`deduct(userId, metrics, idempotencyKey?, metadata?, feature?)` prices a
...
| Method | Parameters | Returns |
| --- | --- | --- |
| `deduct` | `userId: string`, `metrics: UsageMetrics`, `idempotencyKey?`, `metadata?`, `feature?` | `Promise<DeductionResult>` |
| `deductFlatJob` | `userId: string`, `jobName: string`, `idempotencyKey?`, `metadata?`, `feature?` | `Promise<DeductionResult>` |
...
| Method | Parameters | Returns |
| --- | --- | --- |
| `deductTeam` | `teamId: string`, `userId: string`, `metrics: UsageMetrics`, `idempotencyKey?`, `metadata?` | `Promise<TeamDeductionResult>` |
