...
Add credits to a user's account. `options` is `{ type?, metadata?, expiresAt? }` — `type` defaults to `"adjustment"`; `expiresAt` creates time-bounded credits swept by `sweepExpiredCredits()`. Returns `{ transactionId, userId, amount, newBalance, lifetimePurchased }` (money as `Decimal`). Emits `credits.added`. Also re-arms any multi-level low-balance thresholds that the new balance rises above.
