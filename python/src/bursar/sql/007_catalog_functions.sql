...
RETURNS TABLE(revision_id uuid, revision_no bigint, status bursar.catalog_revision_status)
...
CREATE FUNCTION bursar.bucket_expiry_at(
    p_subject_id uuid, p_revision_id uuid, p_bucket_key text
)
RETURNS timestamptz
...
BEGIN
    ...
    IF NOT FOUND THEN RAISE EXCEPTION 'catalog bucket not found' USING ERRCODE = '23503'; END IF;
    IF NOT b.expires THEN RETURN NULL; END IF;
    ...
    ELSIF b.expires_after_anchor = 'plan_assignment' THEN
        ...
        IF NOT FOUND THEN RAISE EXCEPTION 'plan assignment required for bucket expiry' USING ERRCODE = '22023'; END IF;
    ELSE
    ...
    WHILE v_expiry <= now() LOOP
        ...
        IF v_iterations > 10000 THEN RAISE EXCEPTION 'bucket expiry iteration limit exceeded' USING ERRCODE = '22023'; END IF;
    END LOOP;
    RETURN v_expiry;
END $$;
