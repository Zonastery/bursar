...
CREATE FUNCTION bursar.start_plan_migration(
    p_from_plan_id uuid,p_to_plan_id uuid
)
RETURNS uuid
LANGUAGE plpgsql SECURITY DEFINER SET search_path TO '' AS $$
DECLARE v_id uuid;
...
CREATE FUNCTION bursar.migrate_plan_batch(
    p_migration_id uuid,p_batch_size integer DEFAULT 100
)
RETURNS TABLE(migrated integer,done boolean,next_cursor uuid)
LANGUAGE plpgsql SECURITY DEFINER SET search_path TO '' AS $$
DECLARE
...
RETURNS TABLE(change_id uuid,state text,error_code text)
LANGUAGE plpgsql SECURITY DEFINER SET search_path TO '' AS $$
DECLARE
...
BEGIN
    ...
    FOR UPDATE;
    IF NOT FOUND THEN RETURN false; END IF;
    IF c.state=p_state THEN RETURN true; END IF;
