...
CREATE TABLE bursar.billing_entitlement_sources (
    ...
    created_at timestamptz NOT NULL DEFAULT now(),
    UNIQUE (subject_id, subscription_id),
    FOREIGN KEY (subscription_id, subject_id)
        REFERENCES bursar.billing_subscriptions(id, subject_id),
    CHECK ((selected AND selected_at IS NOT NULL AND deselected_at IS NULL)
        OR (NOT selected)),
    CHECK (
...
ON bursar.billing_auto_recharge_attempts(subject_id)
WHERE state IN (
