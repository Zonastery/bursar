...
COMMENT ON TABLE bursar.catalog_operations IS 'Metered operation and pricing-measure definitions for a catalog revision.';
COMMENT ON TABLE bursar.catalog_rate_cards IS 'Operation rate cards, rules, and charge-model snapshots for a catalog revision.';
COMMENT ON TABLE bursar.catalog_credit_policies IS 'Credit consumption, debt, and enforcement policies projected from the catalog.';
COMMENT ON TABLE bursar.catalog_admission_policies IS 'Default concurrent-work admission policies projected from the catalog.';
...
COMMENT ON TABLE bursar.credit_lot_source_allocations IS 'Allocation of lot consumption to individual grant sources inside merged lots.';
...
COMMENT ON TABLE bursar.billing_subscriptions IS 'Provider subscription truth, catalog entitlement context, and independently tracked past-due grace state.';
...
COMMENT ON TABLE bursar.billing_auto_recharge_profiles IS 'Canonical auto-recharge configuration per subject and provider environment.';
...
COMMENT ON FUNCTION bursar.create_lease_for_operation(uuid, text, numeric, text, interval, jsonb, text, jsonb, jsonb, numeric, integer)
...
COMMENT ON FUNCTION bursar.record_subscription_conflict(uuid, text, text, text, text, jsonb)
    IS 'Idempotently record a duplicate provider-subscription admission conflict without falsifying either subscription state.';
...
COMMENT ON FUNCTION bursar.pseudonymize_financial_subject(uuid)
    IS 'Irreversibly remove Bursar-held external identity links and mutable PII while retaining financial reconciliation records.';
