...
--
-- Tier restoration (see 010_credit_tiers.sql): restores tier balances LIFO
-- (reverse priority order), deriving each tier's already-refunded amount from
-- the sum of all prior refunds' own tier_breakdown — never a running counter —
-- so repeated partial refunds compose correctly.
--
-- On success returns: refund_transaction_id, user_id, amount, new_balance,
-- tier_breakdown. All error envelopes also carry user_id + new_balance so the
...
BEGIN
    ...
    -- the tier_breakdown driving LIFO restoration is available.
