...
export class AnalyticsRepository {
  ...
  async spendByUser(start: string, end: string): Promise<SpendByUserRow[]> {
    ...
    return (rows ?? []).map((row) =>
      safeParse(SpendByUserRowSchema, row, "AnalyticsRepository.spendByUser"),
    );
  }
  ...
  async spendByModel(start: string, end: string): Promise<SpendByModelRow[]> {
    ...
    return (rows ?? []).map((row) =>
      safeParse(SpendByModelRowSchema, row, "AnalyticsRepository.spendByModel"),
    );
  }
  ...
  async topUsers(limit: number, start: string, end: string): Promise<TopUserRow[]> {
    ...
    return (rows ?? []).map((row) =>
      safeParse(TopUserRowSchema, row, "AnalyticsRepository.topUsers"),
    );
  }
  ...
  async dailySpend(start: string, end: string): Promise<DailySpendRow[]> {
    ...
    return (rows ?? []).map((row) =>
      safeParse(DailySpendRowSchema, row, "AnalyticsRepository.dailySpend"),
    );
  }
  ...
  async listLedgerEntries(
    ...
    const rows = await this.callproc(usageOnly ? "list_usage_entries" : "list_ledger_entries", [
      userId,
      entryTypes,
      fromDate,
      toDate,
