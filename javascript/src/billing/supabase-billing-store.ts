...
export class SupabaseBillingStore extends BillingStore {
  ...
  async resolveBillingOfferByLookup(
    ...
    const { data, error } = await this.supabase.rpc("resolve_billing_offer_by_lookup", {
      p_provider: provider,
  ...
  async resolveCreditTopupByLookup(
    ...
    const { data, error } = await this.supabase.rpc("resolve_credit_topup_by_lookup", {
      p_provider: provider,
      p_lookup_key: lookupKey,
    });
    if (error) throw error;
  ...
  async getUserSubscriptions(userId: string): Promise<BillingSubscriptionState[]> {
    ...
    if (error) throw error;
    return (data ?? []).map((r) => this.rowToSubscriptionState(r));
  }
  ...
  async deactivateOtherProviderSubscriptions(
    ...
    const { data, error } = await this.supabase.rpc("deactivate_other_provider_subscriptions", {
      p_user_id: userId,
